        ...
        match handle.case().await.as_str() {
           ...
           "spawn" => {
               match cmd.spawn() {
                   Ok(child) => {
                       handle.signal(literal!("ok"));
                       return provide_process(handle, Arc::new(Mutex::new(child))).await;
                   }
                   Err(err) => {
                       handle.signal(literal!("err"));
                       return handle.provide_string(ParString::from(err.to_string()));
...
async fn provide_process(mut handle: Handle, child: Arc<Mutex<std::process::Child>>) {
    loop {
        match handle.case().await.as_str() {
            "info" => {
                provide_processinfo(handle.send(), child.clone());
                continue;
            }
            "wait" => {
...
fn provide_processinfo(handle: Handle, child: Arc<Mutex<std::process::Child>>) {
    handle.provide_box(move |mut handle| {
